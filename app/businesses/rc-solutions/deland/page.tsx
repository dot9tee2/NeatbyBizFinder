...
export const metadata: Metadata = {
  title: `${rcSolutions.name} | Deland AC, Drywall & Water Heaters`,
  description:
    'Deland HVAC installation & repair, drywall/painting, and water heater services. Financing available: $0 down, no payments, no interest for 24 months. Call today.',
  alternates: { canonical: 'https://nearbybizfinder.com/businesses/rc-solutions/deland/' },
  openGraph: {
    title: `${rcSolutions.name} | Deland AC, Drywall & Water Heaters`,
    description:
      'Trusted AC, drywall/painting, and water heater services in Deland. Honest pricing, strong guarantees, and financing with $0 down for qualified customers.',
    url: 'https://nearbybizfinder.com/businesses/rc-solutions/deland/',
    type: 'website'
  },
  twitter: {
    ...
    title: `${rcSolutions.name} | Deland AC, Drywall & Water Heaters`,
    description:
      'Trusted AC, drywall/painting, and water heater services in Deland.'
  }
};
...
function JsonLd() {
  const json = {
    ...
    areaServed: ['Deland', rcSolutions.serviceArea, 'Florida'],
    url: 'https://nearbybizfinder.com/businesses/rc-solutions/deland/',
...
function LocalNavbar() {
  return (
    <header className="sticky top-0 z-40 bg-[#1b1f24]/90 backdrop-blur text-white border-b border-[#2a2f35]">
      <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-14 items-center justify-between">
          <Link href="/businesses/rc-solutions/deland" className="flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <a href="#faqs" className="hover:text-amber-300">FAQs</a>
          ...
          <div className="flex items-center gap-2">
            <Link href={rcSolutions.phoneHref} className="hidden sm:inline-flex">
              <Button className="bg-amber-500 text-[#1b1f24] hover:bg-amber-400">{rcSolutions.phone}</Button>
            </Link>
            <Link href="#contact">
              <Button variant="outline" className="border-amber-300 text-amber-300">Get Estimate</Button>
            </Link>
          </div>
        </div>
      </nav>
...
function Hero() {
  return (
    <section className="relative isolate overflow-hidden bg-gradient-to-br from-[#1b1f24] via-[#0f172a] to-[#0b1020] text-white">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20 md:py-28 relative">
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight">Deland HVAC, Drywall & Water Heater Services</h1>
        <p className="mt-4 max-w-2xl text-slate-200">Reliable repairs and installs with upfront pricing and fast scheduling. Financing with $0 down for qualified customers.</p>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href={rcSolutions.phoneHref}><Button className="bg-amber-500 text-[#1b1f24] hover:bg-amber-400">Call {rcSolutions.phone}</Button></Link>
          <Link href="#contact"><Button variant="outline" className="border-amber-300 text-amber-200">Request Estimate</Button></Link>
        </div>
      </div>
    </section>
  );
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, idx) => (
            <div key={idx} className="relative h-32 md:h-40 rounded-md overflow-hidden">
              <OptimizedImage src={cityImage(rcSolutions.imagesBase, 'deland', `gallery-${idx + 1}.jpeg`)} alt={`Gallery image ${idx + 1}`} fill />
            </div>
          ))}
        </div>
      </div>
...
function ContactCTA() {
  return (
    <section className="bg-gradient-to-r from-[#1b1f24] to-[#0f172a] text-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 flex flex-col md:flex-row items-center gap-6">
        <div className="flex-1">
          <h2 className="text-2xl md:text-3xl font-bold">Get fast help in Deland</h2>
...
function ContactForm() {
  const mailtoHref = `mailto:${encodeURIComponent(rcSolutions.email)}?subject=${encodeURIComponent('Estimate Request — Deland')}`;
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <form action={mailtoHref} method="post" className="mt-6 grid md:grid-cols-2 gap-4" aria-label="Estimate request form">
          ...
          <div className="md:col-span-2">
            ...
            <input type="hidden" name="city" value="Deland" />
          </div>
          <div className="md:col-span-2 flex items-center gap-3">
            <Button type="submit" className="bg-[#1b1f24] hover:bg-[#222831]">Send</Button>
            <Link className="text-amber-600 underline" href={rcSolutions.phoneHref}>Prefer to call? {rcSolutions.phone}</Link>
          </div>
        </form>
      </div>
    </section>
...
function Footer() {
  ...
  return (
    <footer className="bg-[#0f1320] text-slate-300">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="mt-2 text-sm">Deland Service Area</p>
          </div>
